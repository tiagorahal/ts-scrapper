...
EXCEL_PATH = DADOS_DIR / "colegio_recursal_tjsp.xlsx"
...
async def extrair_processos_da_pagina_colegio_recursal(page):
    ...
    for row in rows:
        try:
            ...
            # --- Função de texto (igual seg. instância) ---
            async def txt(sel):
            ...
            # Numeros de 1ª instância (do bloco "Números de 1ª Instância")
            numeros_1inst = []
            try:
            ...
            try:
                ...
                for tr in partes_trs:
                    ...
                    if "Agravante" in tipo:
                        requerente = nome
                    elif "Agravado" in tipo:
                        requerido = nome
            ...
            processo_info.update({
                ...
                "ultima_carga": "",
                "foro": "",    # pode tentar extrair da origem, se quiser
                "vara": "",
                "juiz": "",
            ...
            print(f"❌ Erro ao processar processo Colégio Recursal: {e}")
            try: await detalhe.close()
            except: pass
            continue

async def buscar_colegio_recursal(cnpj):
    ...
    async with async_playwright() as p:
        ...
        print("\n====== COLÉGIO RECURSAL ======\n")
        await page.goto("https://esaj.tjsp.jus.br/cposgcr/open.do")
        ...
        print("⏳ Aguarde 30 segundos (Colégio Recursal)...")
        await page.wait_for_timeout(30000)
        pagina = 1
        while True:
            print(f"🔍 Extraindo dados da página {pagina} (Colégio Recursal)...")
            await extrair_processos_da_pagina_colegio_recursal(page)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python tjsp_colegio_recursal.py <CNPJ>")
        sys.exit(1)
    cnpj_input = sys.argv[1]
    asyncio.run(buscar_colegio_recursal(cnpj_input))